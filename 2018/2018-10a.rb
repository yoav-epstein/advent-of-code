str="position=< 52484, -20780> velocity=<-5,  2>
...
def graph(points)
  118.upto(130) do |y|
    150.upto(240) do |x|
      if points.any? { |point| point.match?(x, y) }
        print '#'
      else
        print '.'
      end
...
pp points.minmax_by { |point| point.x }
pp points.minmax_by { |point| point.y }
