...
events = str.split("\n").map do |event|
  time = Time.parse(event[1..16])
  command = event[19]
  command = event[26..30].to_i if command == 'G'
  [time, command]
end.sort
...
guard = 0
start = 0
events.each do |time, command|
  case command
  when 'f'
    start = time.min
  when 'w'
    hash[guard] += [[start, time.min]]
  else
    guard = command
  end
end
...
sleep = {}
hash.map do |key, times|
  sleep[key] = times.map { |start, stop| stop - start }.sum
end
pp sleep.inspect

guard = sleep.max_by { |k, v| v }.first
pp guard

minutes = Hash.new(0)
hash[guard].each do |start, stop|
  start.upto(stop - 1) { |minute| minutes[minute] += 1 }
end
pp minutes.inspect

minute = minutes.max_by { |k, v| v }.first
pp minute
